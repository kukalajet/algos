/**
 * It may be custom compare function that may compare custom objects together.
 */
type CompareFunction = (a: number | string, b: number | string) => number;

export default class Comparator {
  compare: CompareFunction;

  constructor(compareFunction: CompareFunction) {
    this.compare = compareFunction || Comparator.defaultCompareFunction;
  }

  /**
   * Default comparison function. It just assumes that `a` and `b` are strings
   * or numbers.
   */
  static defaultCompareFunction(
    a: number | string,
    b: number | string
  ): number {
    if (a === b) return 0;
    return a < b ? -1 : 1;
  }

  /**
   * Check if two variables are equal.
   */
  equal(a: number | string, b: number | string): boolean {
    return this.compare(a, b) === 0;
  }

  /**
   * Checks if variable `a` is less than `b`.
   */
  lessThan(a: number | string, b: number | string): boolean {
    return this.compare(a, b) < 0;
  }

  /**
   * Checks if variable `a` is greater than `b`.
   */
  greaterThan(a: number | string, b: number | string): boolean {
    return this.compare(a, b) > 0;
  }

  /**
   * Checks if a variable `a` is less than or equal to `b`.
   */
  lessThanOrEqual(a: number | string, b: number | string): boolean {
    return this.lessThan(a, b) || this.equal(a, b);
  }

  /**
   * Checks if a variable `a` is greater than or equal to `b`.
   */
  greaterThanOrEqual(a: number | string, b: number | string): boolean {
    return this.greaterThan(a, b) || this.equal(a, b);
  }

  /**
   * Reverses the comparison order.
   */
  reverse() {
    const compareOriginal = this.compare;
    this.compare = (a, b) => compareOriginal(b, a);
  }
}
